      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {viewMode === "DAILY" && (
          <div className="flex items-center gap-1 rounded-[var(--radius-pill)] border border-line bg-surface p-1">
            ...
            <button onClick={() => shiftMonth(1)} disabled={currentMonth.getFullYear() === new Date().getFullYear() && currentMonth.getMonth() === new Date().getMonth()} aria-label="Next month" className="rounded-[var(--radius-pill)] p-1.5 text-fg-subtle transition-colors hover:bg-base hover:text-fg disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {viewMode === "MONTHLY" && (
          <div className="flex items-center gap-1 rounded-[var(--radius-pill)] border border-line bg-surface p-1">
            ...
            <button onClick={() => shiftYear(1)} disabled={currentMonth.getFullYear() >= new Date().getFullYear()} aria-label="Next year" className="rounded-[var(--radius-pill)] p-1.5 text-fg-subtle transition-colors hover:bg-base hover:text-fg disabled:opacity-50 disabled:cursor-not-allowed">
              <CaretRight size={16} weight="bold" />
            </button>
          </div>
        )}
                    ...
                    {summary && summary.tradeCount > 0 ? (
                      <div className="mt-auto w-full">
                        <div className={cn("truncate text-xs font-bold sm:text-sm", pnlTone(pnl))}>
                          <DisplayValue type="PNL" money={pnl} compact percent={summary.startBalance ? (pnl / summary.startBalance) * 100 : undefined} />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
                          ...
                          <p className={cn("text-lg font-bold tnum", pnlTone(selectedDayTrades.reduce((acc, t) => acc + Number(t.netPnl ?? 0), 0)))}>
                            <DisplayValue type="PNL" money={selectedDayTrades.reduce((acc, t) => acc + Number(t.netPnl ?? 0), 0)} compact />
                          </p>
                        </div>
                        <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-card)] border border-line bg-surface px-5 py-4">
            <div>
              ...
              <p className={cn("mt-1 text-2xl font-bold tnum tracking-tight", pnlTone(yearTotal.pnl))}>
                <DisplayValue type="PNL" money={yearTotal.pnl} compact />
              </p>
            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
                            ...
                            <p className={cn("text-lg font-bold tnum", pnlTone(selectedMonthMeta.pnl))}>
                              <DisplayValue type="PNL" money={selectedMonthMeta.pnl} compact />
                            </p>
                          </div>
                          <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
                            <p className="mb-1 text-xs text-fg-subtle">Trades</p>
                            ...
                            {selectedMonthDays.map((d) => (
                              ...
                              >
                                ...
                                <div className={cn("shrink-0 text-sm font-bold tnum", pnlTone(d.pnl))}>
                                  <DisplayValue type="PNL" money={d.pnl} compact percent={d.startBalance ? (d.pnl / d.startBalance) * 100 : undefined} />
                                </div>
                              </button>
                            ))}
                          </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3">
              {yearlyGrid.map(({ yearStr, pnl, tradeCount }, i) => {
                ...
                return (
                  ...
                  >
                    <span className={cn("mb-1 text-xs font-bold uppercase tracking-widest text-fg-subtle")}>{yearStr}</span>
                    {active ? (
                      <div className="mt-auto w-full">
                        <div className={cn("truncate text-xs font-bold sm:text-sm", pnlTone(pnl))}>
                          <DisplayValue type="PNL" money={pnl} compact />
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
                          ...
                          <p className={cn("text-lg font-bold tnum", pnlTone(selectedYearMeta.pnl))}>
                            <DisplayValue type="PNL" money={selectedYearMeta.pnl} compact />
                          </p>
                        </div>
                        <div className="rounded-[var(--radius-control)] border border-line bg-base p-3">
                          <p className="mb-1 text-xs text-fg-subtle">Trades</p>
                          ...
                          {selectedYearMonths.map((m) => (
                            ...
                            >
                              ...
                              <div className={cn("shrink-0 text-sm font-bold tnum", pnlTone(m.pnl))}>
                                <DisplayValue type="PNL" money={m.pnl} compact />
                              </div>
                            </button>
                          ))}
                        </div>
