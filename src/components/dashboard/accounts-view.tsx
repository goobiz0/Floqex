    ...
    <div className="space-y-6">
      ...
      {accounts.length === 0 ? (
        ...
        <>
          ...
          {(query || mode !== "ALL") && visible.length > 0 && (
            <p className="text-center text-xs text-fg-subtle">
              Showing <span className="tnum text-fg-muted">{visible.length}</span> of{" "}
              <span className="tnum text-fg-muted">{accounts.length}</span> accounts
            </p>
          )}
        </>
      )}
