...
export const FeatureStrategy = () => {
  ...
  const getEntrance = (startFrame: number) => {
    const opacity = interpolate(frame, [startFrame, startFrame + 20], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });
    const y = interpolate(frame, [startFrame, startFrame + 20], [12, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });
    return { opacity, transform: `translateY(${y}px)` };
  };

  const block1 = getEntrance(10);
  const block2 = getEntrance(55);
  const block3 = getEntrance(100);

  const line1PathLength = interpolate(frame, [30, 55], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.4, 0, 0.2, 1) });
  const line2PathLength = interpolate(frame, [75, 100], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.4, 0, 0.2, 1) });

  const pulseOpacity = interpolate(frame, [120, 140, 160], [0.3, 0.8, 0.3], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "transparent", fontFamily: '"Outfit", system-ui, sans-serif', padding: 24, opacity: overallOpacity }}>
      ...
      <div style={{ position: "relative", flex: 1, marginTop: 20 }}>
        ...
        {/* Glow for block 3 */}
        {frame >= 120 && (
          <div style={{ position: "absolute", left: 100, top: 135, width: 100, height: 35, borderRadius: RADIUS.control, backgroundColor: COLORS.accent, filter: "blur(12px)", opacity: pulseOpacity }} />
        )}
