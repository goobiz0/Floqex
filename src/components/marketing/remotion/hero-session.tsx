...
export const HeroSession = () => {
  ...
  const shellOpacity = interpolate(frame, [0, 20], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.16, 1, 0.3, 1),
  });

  // Act 2: Cursor moves and clicks Start (30-90)
  const cursorOpacity = interpolate(frame, [30, 40, 230, 240], [0, 1, 1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const cursorX = interpolate(frame, [40, 80, 130, 160], [280, 235, 235, 400], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.23, 1, 0.32, 1),
  });
  const cursorY = interpolate(frame, [40, 80, 130, 160], [210, 245, 245, 180], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.23, 1, 0.32, 1),
  });
  const isClicking = frame >= 85 && frame <= 95;

  // Button brightness on click
  const buttonOpacity = interpolate(frame, [85, 90, 95], [1, 0.7, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // Status pill appears (95-110)
  const statusOpacity = interpolate(frame, [95, 110], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.16, 1, 0.3, 1),
  });
  const statusY = interpolate(frame, [95, 110], [10, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.16, 1, 0.3, 1),
  });

  // Chart line draws (95-180)
  const pathLength = interpolate(frame, [95, 180], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  ...
  // Feed items staggered (120, 140, 160, 180)
  const getFeedItemStyle = (startFrame: number) => {
    ...
    const x = interpolate(frame, [startFrame, startFrame + 15], [15, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.bezier(0.16, 1, 0.3, 1),
    });
  ...
  const feed1 = getFeedItemStyle(120);
  const feed2 = getFeedItemStyle(140);
  const feed3 = getFeedItemStyle(160);
  const feed4 = getFeedItemStyle(180);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.base, fontFamily: '"Outfit", system-ui, sans-serif' }}>
      <div style={{ opacity: shellOpacity, display: "flex", height: "100%", width: "100%" }}>
        {/* Left thin sidebar */}
        <div style={{ width: 48, height: "100%", borderRight: `1px solid ${COLORS.line}`, backgroundColor: COLORS.elevated, display: "flex", flexDirection: "column", alignItems: "center", padding: "16px 0", gap: 16 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          {/* Mini Topbar */}
          <div style={{ height: 48, borderBottom: `1px solid ${COLORS.line}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px" }}>
          ...
          <div style={{ flex: 1, padding: 20, display: "flex", gap: 20 }}>
            ...
            <div style={{ flex: 2, display: "flex", flexDirection: "column", gap: 20 }}>
              <MockCard style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1.5, display: "flex", flexDirection: "column" }}>
              <MockCard style={{ flex: 1, padding: 16 }}>
                <span style={{ color: COLORS.fgSubtle, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 16 }}>Live Feed</span>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {frame >= 120 && <FeedItem label="Scanning EURUSD" meta="1m bars" style={feed1} />}
                  {frame >= 140 && <FeedItem label="Signal detected" meta="Breakout rule" style={feed2} />}
                  {frame >= 160 && <FeedItem label="Risk check passed" meta="Within limits" iconColor={COLORS.info} style={feed3} />}
                  {frame >= 180 && <FeedItem label="Order placed" meta="Long, bracketed" style={feed4} />}
                </div>
              </MockCard>
            </div>
          </div>
