...
export const FeatureJournal = () => {
  ...
  const rowOpacity = interpolate(frame, [20, 35], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const rowY = interpolate(frame, [20, 35], [10, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });

  const detailOpacity = interpolate(frame, [40, 60], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const detailHeight = interpolate(frame, [40, 60], [0, 100], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.23, 1, 0.32, 1) });

  const badgeOpacity = interpolate(frame, [70, 80], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const badgeScale = interpolate(frame, [70, 80], [0.9, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.base, fontFamily: '"Outfit", system-ui, sans-serif', padding: 24, opacity: overallOpacity }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", backgroundColor: COLORS.elevated, border: `1px solid ${COLORS.line}`, borderRadius: RADIUS.card, padding: 16 }}>
        {/* Header row */}
        ...
        {/* Badge */}
        {frame >= 70 && (
          <div style={{ position: "absolute", bottom: 20, right: 20, opacity: badgeOpacity, transform: `scale(${badgeScale})`, backgroundColor: COLORS.surface, border: `1px solid ${COLORS.line}`, color: COLORS.fgSubtle, fontSize: 10, fontWeight: 500, padding: "4px 10px", borderRadius: RADIUS.pill }}>
