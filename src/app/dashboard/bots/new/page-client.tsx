...
export function BotsNewClient({ 
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl pb-28">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-max animate-in slide-in-from-bottom-5 fade-in duration-300">
        <div className="flex items-center gap-4 rounded-full border border-line bg-surface/90 px-6 py-3 shadow-2xl backdrop-blur-md">
          <button 
            type="button" 
          ...
          <button
            ...
            className="flex h-9 items-center gap-2 rounded-full bg-accent px-5 text-sm font-bold text-[var(--color-on-accent)] transition-transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:pointer-events-none shadow-md shadow-accent/20"
          >
            {loading ? (
              <>
